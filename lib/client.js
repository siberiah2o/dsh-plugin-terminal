...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* ── minimal line-oriented terminal emulation ─────────────────────────
		 * A tiny state machine over the raw PTY byte stream. It models what a
		 * real text terminal does at line granularity:
		 *   - \r          moves the cursor to column 0 and OVERWRITES (the
		 *                  redraw primitive used by progress bars and
		 *                  PSReadLine prompts — must not become a newline);
		 *   - \n          commits the current line;
		 *   - CSI K       erases from the cursor to end of line;
		 *   - CSI 2J      erases the whole screen;
		 *   - OSC ... BEL skips title/bell sequences.
		 * Everything else is plain text. State lives in a mutable model so
		 * chunks apply incrementally and the rendered text is O(buffer).
		 */
		function TerminalModel() {
			...
			let col = 0;
			let prev = "";
			// Newline style, detected at the first newline seen: CRLF streams
			// (Windows ConPTY) may contain stray lone LFs as rendering residue
			// (e.g. "\r\n\n\n"); LF streams (Unix) treat every LF literally.
			let crlfStyle = null;
			// After CSI 2J (clear screen), LFs are cursor-down placeholders until
			// real content arrives (PSReadLine positions the prompt this way).
			let cleared = false;
			const write = (raw) => {
				let i = 0;
				while (i < raw.length) {
					...
					if (ch === "\x1b") {
						const next = raw[i + 1];
						if (next === "[") {
							// CSI: parse params until the final byte @-~
							let j = i + 2;
							let params = "";
							while (j < raw.length && !/[ @-~]/.test(raw[j])) {
								params += raw[j];
								j++;
							}
							const final = raw[j];
							...
							} else if (final === "J" && params.includes("2")) {
								lines.length = 0; // erase entire screen
								line = "";
								col = 0;
								cleared = true;
							}
							...
							i += 2; // single-byte escapes (\x1b=, \x1b>, ...)
						}
						continue;
					}
					if (ch === "\n") {
						if (crlfStyle === null) crlfStyle = prev === "\r";
						if (cleared) {
							// cursor-down placeholder after clear-screen — no visible line
						} else if (crlfStyle && prev !== "\r" && line.length === 0 && col === 0) {
							// CRLF stream, empty line, stray lone LF: ConPTY residue — ignore
						} else {
							lines.push(line);
							line = "";
							col = 0;
						...
						col = 0; // carriage return: back to column 0, overwrite
					} else if (ch === "\t") {
						line = line.slice(0, col) + "    " + line.slice(col + 4);
						col += 4;
			...
			return {
				write,
				text: () => (lines.length === 0 ? line : lines.join("\n") + (line.length > 0 ? "\n" + line : "")),
			};
		}
		...
		function TerminalPanel() {
			...
			const attach = React.useCallback((id) => {
				...
				es.addEventListener("data", (ev) => {
					const text = String(JSON.parse(ev.data));
			...
			const boot = React.useCallback(async (forceNew) => {
				setBusy(true);
				try {
					let s = null;
					if (!forceNew) {
			...
			const restart = React.useCallback(async () => {
				// kill the current session first so boot() cannot reuse it
				if (sessionId !== null) {
			...
			// render the raw chunk buffer through the line-oriented terminal model
			...
			return React.createElement(
				...
				open
					? React.createElement(
						...
						React.createElement("pre", { className: "dshTermOut", ref: outRef }, displayText),
